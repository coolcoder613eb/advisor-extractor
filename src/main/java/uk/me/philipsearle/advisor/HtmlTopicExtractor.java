...
public class HtmlTopicExtractor {
    ...
    public static void main(String[] args)
        ...
        AdvisorDocumentLoader documentLoader = new AdvisorDocumentLoader(
            Charset.forName("CP437")
        );
        AdvisorDocument advisorDocument = documentLoader.loadDocument(
            inputFile
        );

        outputDocumentSummary(
            advisorDocument,
            new File(outputDirectory, "_SUMMARY.TXT")
        );
        for (HelpTopic topic : advisorDocument.getTopics()) {
            ...
            outputTopicHtml(
                ...
                new File(
                    outputDirectory,
                    "TOPIC_" + topic.getLocalContextId() + ".HTML"
                )
            );
        }
    }
    ...
    private static String getRunningJarFile() throws URISyntaxException {
        CodeSource codeSource =
            HtmlTopicExtractor.class.getProtectionDomain().getCodeSource();
        File jarFile = new File(codeSource.getLocation().toURI().getPath());
        return jarFile.getParentFile().getPath();
    }

    private static void outputDocumentSummary(
        ...
        try (
            ...
            out.printf(
                "Max display width    : %d characters\n",
                advisorDocument.getMaxDisplayWidth()
            );
            out.printf(
                "Application prefix   : '%c' (%d)\n",
    ...
    static void outputTopicHtml(
        ...
        try (
            ...
            for (HelpTopicLine line : topic.getText()) {
                out.println(
                    line.getHtmlFormattedText(document.getGlobalContextMap())
                );
            }
            ...
            throw new RuntimeException(
                "Failed to write topic " + topic.getLocalContextId(),
                e
            );
        }
    }
